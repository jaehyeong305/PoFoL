...
const MainChallengeComponent: React.FC = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (scrollY >= 3750 && scrollY <= 4900) {
                const y = 42 - ((scrollY - 3750) / (4900 - 3750)) * (42 + 65);
                setTransformY(y);
            }
            
            // NOTE(hajae): 텍스트가 올라가며 없어질 때, 투명도를 높여 없어지는 듯이 표시한다.
            if (scrollY >= 4300 && scrollY <= 4700) {
                const textOpacity = 1 - (scrollY - 4300) / (4700 - 4300);
        ...
        <div className="main-challenge">
            ...
            <div className={`description ${isImageFixed ? "current" : ""}`}>
                ...
                <p className="text-box">
                    ...
                    <span style={{transform: `translate3d(0px, ${transformY}px, 0px)`}}>[TODO] 문구는 나중에 한번 생각해보도록 하자</span>
                </p>
            </div>
        </div>
    );
